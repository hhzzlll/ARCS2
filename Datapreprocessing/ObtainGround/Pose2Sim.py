...
## FUNCTIONS
def setup_logging(session_dir, project_dir=None):
    '''
    Create logging file and stream handlers
    
    Prefer writing logs to project_dir/logs.txt when project_dir is provided,
    otherwise fallback to session_dir/logs.txt.
    '''

    try:
        log_dir = os.path.realpath(project_dir) if project_dir else session_dir
    ...
    logging.basicConfig(
        ...
        level=logging.INFO,
        handlers=[
            logging.handlers.TimedRotatingFileHandler(os.path.join(log_dir, 'logs.txt'), when='D', interval=7),
...
class Pose2SimPipeline:
    def __init__(self, config=None):
        ...
        if not use_custom_logging:
            # Write session logs into the active project directory if available
            try:
                first_project_dir = self.config_dicts[0].get('project').get('project_dir')
            except Exception:
                first_project_dir = None
            setup_logging(self.session_dir, first_project_dir)
    ...
    def calibration(self):
        ...
        # Prefer explicit project_dir/calibration if present
        project_dir = os.path.realpath(config_dict.get('project').get('project_dir'))
        candidate_paths = []
        # 1) Standard location under project_dir
        candidate_paths.append(os.path.join(project_dir, 'calibration'))
        # 2) Any folder in session_dir containing 'calib'
        try:
        ...
        for p in candidate_paths:
            if os.path.isdir(p):
                # Require either intrinsics/extrinsics subfolders or a toml file inside
                has_intr = os.path.isdir(os.path.join(p, 'intrinsics'))
                has_extr = os.path.isdir(os.path.join(p, 'extrinsics'))
                has_toml = any(fn.lower().endswith('.toml') for fn in os.listdir(p)) if os.path.isdir(p) else False
                if has_intr or has_extr or has_toml:
                    calib_dir = p
                    break

        if calib_dir is None:
            logging.error('Could not find the calibration folder or files.')
            logging.error(f"Tried: {candidate_paths}")
