...
from Datapreprocessing.ObtainGround.common import retrieve_calib_params, computeP, weighted_triangulation, \
...
from Datapreprocessing.ObtainGround.skeletons import *
...
## FUNCTIONS
def _resolve_calib_dir(project_dir, session_dir):
    """Return a valid calibration directory.

    Priority:
    1) project_dir/calibration (if contains intrinsics/extrinsics or any .toml)
    2) session_dir/Data/<YYYYMMDD>/calibration (newest)
    3) any folder in session_dir containing 'calib'
    """
    # 1) Preferred: project_dir/calibration
    cand1 = os.path.join(project_dir, 'calibration')
    if os.path.isdir(cand1):
        has_toml = len(glob.glob(os.path.join(cand1, '*.toml'))) > 0
        has_intr = os.path.isdir(os.path.join(cand1, 'intrinsics'))
        has_extr = os.path.isdir(os.path.join(cand1, 'extrinsics'))
        if has_toml or has_intr or has_extr:
            return cand1

    # 2) Data/<YYYYMMDD>/calibration under session_dir/Data
    try:
        data_dir = os.path.join(session_dir, 'Data')
        numeric_dirs = [d for d in os.listdir(data_dir)
                        if os.path.isdir(os.path.join(data_dir, d)) and re.fullmatch(r'\d{8}', d)]
        calib_dirs = [os.path.join(data_dir, d, 'calibration')
                      for d in numeric_dirs
                      if os.path.isdir(os.path.join(data_dir, d, 'calibration'))]
        if calib_dirs:
            calib_dirs.sort(key=os.path.getctime, reverse=True)
            return calib_dirs[0]
    except Exception:
        pass

    # 3) Fallback: any folder in session_dir containing 'calib'
    try:
        candidates = [os.path.join(session_dir, c) for c in os.listdir(session_dir)
    ...
    raise Exception('No calibration directory found. Checked project_dir/calibration, Data/<YYYYMMDD>/calibration, and session_dir/*calib*')


def _latest_calib_file(calib_dir: str) -> str:
    calib_files = glob.glob(os.path.join(calib_dir, '*.toml'))
    if not calib_files:
        raise Exception(f'No .toml calibration file found in the {calib_dir}.')
    return max(calib_files, key=os.path.getctime)
...
def make_trc(config_dict, Q, keypoints_names, f_range, id_person=-1):
    ...
    try:
        ...
        logging.warning(f"Failed to save ground TRC/CSV copies: {e}")
...
def recap_triangulate(config_dict, error, nb_cams_excluded, keypoints_names, cam_excluded_count, interp_frames, non_interp_frames, f_range_trimmed, trc_path):
    ...
    calib_dir = _resolve_calib_dir(project_dir, session_dir)
    calib_file = _latest_calib_file(calib_dir)
